...
public class TopologicalSort
{
  ...
  public TopologicalSort()
  {
    ...
    this.written_strs = new ArrayList();
    
    RefactoringQuery move_field = new RefactoringQuery(
      "move_field", 
      "deleted_field(?fFullName, ?fShortName, ?tFullName), added_field(?f1FullName, ?fShortName, ?t1FullName), deleted_accesses(?fFullName, ?mFullName), added_accesses(?f1FullName, ?nFullName), before_type(?tFullName, ?, ?package), after_type(?t1FullName, ?, ?package), NOT(equals(?tFullName, ?t1FullName))");
    ...
    this.graph.add(move_field_node);
    
    Node move_method_node = new Node(
      new MoveMethod().getRefactoringQuery());
    this.graph.add(move_method_node);
    
    Node rename_method_node = new Node(
      new RenameMethod().getRefactoringQuery());
    this.graph.add(rename_method_node);
    
    String push_down_field_query = "move_field(?fShortName, ?tParentFullName, ?tChildFullName), before_subtype(?tParentFullName, ?tChildFullName)";
    ...
    this.graph.add(push_down_field_node);
    
    String push_down_method_query = "move_method(?mShortName, ?tParentFullName, ?tChildFullName), before_subtype(?tParentFullName, ?tChildFullName)";
    ...
    this.graph.add(push_down_method_node);
    
    Node extract_method_node = new Node(
      new ExtractMethod().getRefactoringQuery());
    this.graph.add(extract_method_node);
    ...
    this.graph.add(decompose_conditional_node);
    
    String pull_up_field_query = "move_field(?fShortName, ?tChildFullName, ?tParentFullName), before_subtype(?tParentFullName, ?tChildFullName)";
    ...
    this.graph.add(pull_up_field_node);
    
    String pull_up_method_query = "move_method(?mShortName, ?tChildFullName, ?tParentFullName), before_subtype(?tParentFullName, ?tChildFullName)";
    ...
    this.graph.add(pull_up_method_node);
    ...
    this.graph.add(collapse_hierarchy_node);
    ...
    this.graph.add(remove_parameter_node);
    
    Node inline_method_node = new Node(
      new InlineMethod().getRefactoringQuery());
    this.graph.add(inline_method_node);
    ...
    this.graph.add(add_parameter_node);
    
    Node replace_method_with_method_object_node = new Node(
      new ReplaceMethodWithMethodObject().getRefactoringQuery());
    this.graph.add(replace_method_with_method_object_node);
    
    String extract_class_query = "added_type(?newtFullName, ?newtShortName, ?),before_type(?tFullName, ?tShortName, ?pkg),after_type(?tFullName, ?tShortName, ?pkg),added_field(?fFullName, ?, ?tFullName),added_fieldoftype(?fFullName, ?newtFullName),(move_field(?fShortName, ?tFullName, ?newtFullName);move_method(?mShortName, ?tFullName, ?newtFullName))";
    ...
    this.graph.add(inline_class_node);
    
    String hide_delegate_query = "after_method(?clientmFullName, ?, ?clienttFullName),deleted_calls(?clientmFullName, ?delegatemFullName),added_calls(?clientmFullName, ?servermFullName),added_method(?servermFullName, ?, ?servertFullName),added_calls(?servermFullName, ?delegatemFullName),before_field(?delegatefFullName, ?, ?servertFullName),after_field(?delegatefFullName, ?, ?servertFullName),before_fieldoftype(?delegatefFullName, ?delegatetFullName),after_fieldoftype(?delegatefFullName, ?delegatetFullName),after_method(?delegatemFullName, ?, ?delegatetFullName),NOT(equals(?clienttFullName, ?servertFullName)),NOT(equals(?delegatetFullName, ?servertFullName))";
    ...
    this.graph.add(remove_middle_man_node);
    ...
    this.graph.add(introduce_local_extension_node);
    
    String replace_ctor_with_factory_method_query = "added_method(?mFactFullName, ?, ?tFullName),after_method(?mCtorFullName,\"<init>()\",?tFullName),added_calls(?mFactFullName, ?mCtorFullName),added_methodmodifier(?mCtorFullName,\"private\")";
    ...
    this.graph.add(replace_ctor_with_factory_method_node);
    
    Node replace_data_value_with_object_node = new Node(
      new ReplaceDataValueWithObject().getRefactoringQuery());
    this.graph.add(replace_data_value_with_object_node);
    
    RefactoringQuery replace_magic_number_with_const = new RefactoringQuery(
      "replace_magic_number_with_constant", 
      "added_field(?fFullName, ?, ?tFullName),added_fieldmodifier(?fFullName, \"final\"),before_method(?mFullName, ?, ?tFullName), added_accesses(?fFullName, ?mFullName)");
    ...
    Node replace_param_with_method_node = new Node(
      replace_param_with_method);
    this.graph.add(replace_param_with_method_node);
    
    RefactoringQuery hide_method = new RefactoringQuery("hide_method", 
      "deleted_methodmodifier(?mFullName, \"public\"),added_methodmodifier(?mFullName, \"private\")");
    
    hide_method.addType("?mFullName");
    Node hide_method_node = new Node(hide_method);
    this.graph.add(hide_method_node);
    
    RefactoringQuery pull_up_ctor_body = new RefactoringQuery(
      "pull_up_constructor_body", 
      "added_method(?supermFullName, \"<init>()\", ?supertFullName),before_subtype(?supertFullName, ?subtFullName),after_subtype(?supertFullName, ?subtFullName),before_method(?submFullName, \"<init>()\", ?subtFullName),after_method(?submFullName, \"<init>()\", ?subtFullName),added_calls(?submFullName, ?supermFullName)");
    
    pull_up_ctor_body.addType("?supertFullName");
    Node pull_up_ctor_body_node = new Node(pull_up_ctor_body);
    this.graph.add(pull_up_ctor_body_node);
    
    RefactoringQuery extract_subclass = new RefactoringQuery(
      "extract_subclass", 
      "added_subtype(?superTFullName, ?tFullName),NOT(before_type(?tFullName, ?, ?)),(move_field(?fShortName, ?superTFullName, ?tFullName);move_method(?mShortName, ?superTFullName, ?tFullName))");
    ...
    this.graph.add(extract_subclass_node);
    
    RefactoringQuery extract_superclass = new RefactoringQuery(
      "extract_superclass", 
      "added_subtype(?tFullName, ?subtFullName),NOT(before_type(?tFullName, ?, ?)),(move_field(?fShortName, ?subtFullName, ?tFullName);move_method(?mShortName, ?subtFullName, ?tFullName))");
    ...
    this.graph.add(extract_superclass_node);
    ...
    this.graph.add(consolidate_cond_expression_node);
    
    RefactoringQuery extract_interface = new RefactoringQuery(
      "extract_interface", 
      "added_type(?interfacetFullName, ?,?),added_implements(?interfacetFullName, ?othertFullName),before_type(?othertFullName, ?, ?)");
    ...
    this.graph.add(extract_interface_node);
    
    Node replace_exception_with_test_node = new Node(
      new ReplaceExceptionWithTest().getRefactoringQuery());
    this.graph.add(replace_exception_with_test_node);
    
    RefactoringQuery change_value_to_reference = new RefactoringQuery(
      "change_value_to_reference", 
      "replace_constructor_with_factory_method(?mCtorFullName, ?mFactFullName),added_method(?mFactFullName, ?, ?tFullName),before_fieldoftype(?, ?tFullName),after_fieldoftype(?, ?tFullName),NOT(after_method(?, \"equals()\", ?tFullName);after_method(?, \"hashCode()\", ?tFullName))");
    ...
    this.graph.add(change_value_to_reference_node);
    ...
    change_reference_to_value_node
      .addChild(replace_ctor_with_factory_method_node);
    this.graph.add(change_reference_to_value_node);
    
    Node consolidate_duplicate_cond_fragments_node = new Node(
      new ConsolidateDuplicateConditionalFragment()
      .getRefactoringQuery());
    this.graph.add(consolidate_duplicate_cond_fragments_node);
    
    RefactoringQuery encapsulate_downcast = new RefactoringQuery(
      "encapsulate_downcast", 
      "added_cast(?, ?tFullName, ?mFullName),added_return(?mFullName, ?tFullName),deleted_return(?mFullName, ?oldtFullName),(after_subtype(?oldtFullName, ?tFullName);(after_subtype(?oldtFullName, ?othertFullName),after_subtype(?othertFullName, ?tFullName)))");
    ...
    this.graph.add(tease_apart_inheritance_node);
    
    Node introduce_null_object_node = new Node(
      new IntroduceNullObject().getRefactoringQuery());
    this.graph.add(introduce_null_object_node);
    
    RefactoringQuery replace_inheritance_with_delegation = new RefactoringQuery(
      "replace_inheritance_with_delegation", 
      "deleted_subtype(?delegate, ?delegatingObj),added_fieldoftype(?fFullName, ?delegate),added_field(?fFullName, ?, ?delegatingObj)");
    ...
    Node replace_inheritance_with_delegation_node = new Node(
      replace_inheritance_with_delegation);
    this.graph.add(replace_inheritance_with_delegation_node);
    
    RefactoringQuery replace_delegation_with_inheritance = new RefactoringQuery(
      "replace_delegation_with_inheritance", 
      "added_subtype(?delegate, ?delegatingObj), deleted_fieldoftype(?fFullName, ?delegate), deleted_field(?fFullName, ?, ?delegatingObj)");
    ...
    this.graph.add(remove_assignment_to_parameters_node);
    
    RefactoringQuery encapsulate_field = new RefactoringQuery(
      "encapsulate_field", 
      "deleted_fieldmodifier(?fFullName,\"public\"),added_fieldmodifier(?fFullName,\"private\"),added_getter(?mGetFullName, ?fFullName),added_setter(?mSetFullName, ?fFullName)");
    
    encapsulate_field.addType("?fFullName");
    Node encapsulate_field_node = new Node(encapsulate_field);
    this.graph.add(encapsulate_field_node);
    
    RefactoringQuery remove_setting_method = new RefactoringQuery(
      "remove_setting_method", 
      "added_fieldmodifier(?fFullName,\"final\"),deleted_setter(?mFullName,?fFullName)");
    ...
    this.graph.add(remove_setting_method_node);
    
    RefactoringQuery replace_error_code_with_exception = new RefactoringQuery(
      "replace_error_code_with_exception", 
      "deleted_return(?mFullName, ?oldReturnType),added_return(?mFullName, \"void\"),added_throws(?mFullName, ?tFullName)");
    ...
    Node replace_error_code_with_exception_node = new Node(
      replace_error_code_with_exception);
    this.graph.add(replace_error_code_with_exception_node);
    ...
    this.graph.add(introduce_parameter_object_node);
    ...
    Node replace_conditional_with_polymorphism_node = new Node(
      ...
      .getRefactoringQuery());
    this.graph.add(replace_conditional_with_polymorphism_node);
    
    RefactoringQuery self_encapsulate_field = new RefactoringQuery(
      "self_encapsulate_field", 
      "added_getter(?mGetFullName, ?fFullName),before_field(?fFullName, ?, ?), added_setter(?mSetFullName, ?fFullName)");
    ...
    this.graph.add(self_encapsulate_field_node);
    
    RefactoringQuery extract_hierarchy = new RefactoringQuery(
      "extract_hierarchy", 
      "replace_type_code_with_subclasses(?tFullName);replace_type_code_with_state(?tFullName, ?);(replace_conditional_with_polymorphism(?mFullName, ?),before_method(?mFullName, ?, ?tFullName))");
    ...
    replace_subclass_with_field_node
      .addChild(replace_ctor_with_factory_method_node);
    this.graph.add(replace_subclass_with_field_node);
  ...
  public void inferRefactoring(File fileName, Node curr)
  {
    ...
    try
    {
      Connection con = new Connection(this.frontend);
      ...
      while (rs.next())
      {
        Rule currentRule = this.ruleFactory.returnRuleByName(refactoring
          .getName());
        if (currentRule != null)
        {
          ...
          "self_encapsulate_field"))
        {
          ...
          if (!rs2.next())
          {
            checkForStragglers = 
              ...
              "added_accesses(\"" + fieldName + "\", ?mFullName), NOT(added_getter(?mFullName, \"" + fieldName + "\");added_setter(?mFullName, \"" + fieldName + "\"))";
    ...
    throws TyrubaException
  {
    ...
    {
      types_count++;
      String type = (String)types_itr.next();
  ...
  private void storeResult(File fileName, Node curr, ResultSet rs, String result)
    throws IOException, ParseException, TypeModeError
  {
    if ((result != null) && (!this.written_strs.contains(result)))
    {
      this.written_strs.add(result);
      curr.incrementNumFound();
      String pred = "\n" + result + ".";
      List<String> vars1 = substituteInQueryString(curr.getRefQry()
        .getQuery(), rs);
      this.dependents.put(result, vars1);
    ...
    throws IOException, ParseException, TypeModeError
  {
    if (this.frontend == null) {
      if (this.dbDir == null) {
        this.frontend = new FrontEnd(this.loadInitFile, MetaInfo.fdbDir, 
          false, null, true, this.backgroundPageCleaning);
      } else {
